...
class TimelineTable
{
    ...
    {
        $id  = (int) $id;
        $rowset = $this->tableGateway->select(array('idtimeline' => $id));
        $row = $rowset->current();
        if (!$row) {
    ...
    public function saveTimeline(Timeline $timeline)
    {
        ...
        if ($id == 0) 
        {
            ...
            if ($this->getTimeline($id)) {
                $this->tableGateway->update($data, array('idtimeline' => $id));
            } else {
    ...
    {
        $this->tableGateway->delete(array('idtimeline' => (int) $id));
    }
}